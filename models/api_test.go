...
func ServerTest() (*fiber.App, *mongo.Collection, error) {
	...
	env := &TestEnv{ //not to be confused with the poorly named ".env" file that is totally unrelated
		users: models.UsersModel{DB: collection_test},
        emails: models.EmailModel{DB: collection_test},
        files: models.FileModel{DB: collection_test},
	}
	// new fiber app initialized
	app := fiber.New()
	// Routes
	app.Get("/api/test_users", env.users.GetUsers)
	app.Post("/api/test_users", env.users.AddUser)
	app.Delete("/api/test_users/:id", env.users.DeleteUser)
...
func TestUserFunctions(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/api/test_users", bytes.NewReader(userData))
	...
	// check User GET
	req = httptest.NewRequest(http.MethodGet, "/api/test_users", nil)
	resp, err = fiberapp.Test(req)
	assert.Nil(t, err)
